...
class StructuralAttribute(SiteAttribute):
    ...
    def get_rdf(self, cutoff=20.0, bin_size=0.1):
        """
        ...
        neighbors_lst = self.structure.get_all_neighbors(cutoff)
        ...
        number_density = self.structure.num_sites / self.structure.volume
        rdf = dist_hist / shell_vol / number_density

        return rdf, dist_bins[:-1]

    def get_prdf(self, cutoff=20.0, bin_size=0.1):
        """
        ...
        composition = self.structure.composition.fractional_composition.to_reduced_dict
        ...
        for key, distances in distances_by_type.items():
            ...
            # Normalize
            n_alpha = composition[key[0]] * self.structure.num_sites
    ...
    def get_rdf_peaks(self, rdf, rdf_bins, n_peaks=2):
        """
    ...
    def get_redf(self, cutoff=None, dr=0.05):
        """
        ...
        # make primitive
        struct = SpacegroupAnalyzer(self.structure).find_primitive() or self.structure
    ...
    def get_coulomb_matrix(self, diag_elems=False):
        """
        ...
        """
        m = [[] for s in self.structure.sites]
        z = []
        ...
        for i in range(self.structure.num_sites):
            for j in range(self.structure.num_sites):
                if i == j:
                    if diag_elems:
