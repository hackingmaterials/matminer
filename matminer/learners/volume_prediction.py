...
class VolumePredictor:
    ...
    def __init__(self, cutoff=4, ionic_factor=0.30):
        ...
        :param cutoff: (float) cutoff for site pairs (added to site radius)
            in Angstrom. Increase if your initial structure guess is extremely
            bad (atoms way too far apart). In all other cases, increasing
            cutoff gives same answer but takes more time.
        :param ionic_factor: (float) Factor that accounts for ionicity in a
            bond. It determines the contribution of ionic and atomic radii of
            each element making up a bond to the sum of their radii.
        """
    ...
    def predict(self, structure):
        ...
        ionic_mix = get_std(get_element_data(structure.composition, 'X')) * \
                    self.ionic_factor
    ...
    def get_predicted_structure(self, structure):
        ...
        volume using the "predict" method.
        :param structure: pymatgen Structure object
        :return: scaled pymatgen Structure object
        """
