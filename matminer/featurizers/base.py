...
class HeterogeneousFeaturizer(BaseFeaturizer):
    """Class that runs featurizers that may not return the same features for
    all input data (e.g. featurizers that return site specific information)"""
    ...
    def featurize_dataframe(self, df, col_id, ignore_errors=False,
        ...
        Args:
            ...
            inplace (bool): Whether to add new columns to input dataframe (df)
            n_jobs (int): Number of parallel processes to execute when
                featurizing the dataframe. If None, automatically determines
                the number of processing cores on the system and sets n_procs
                to this number.
        Returns:
            updated Dataframe
        """
        ...
        # Check names to avoid overwriting the current columns
        for col in df.columns.values:
            if col in features.columns.values:
                raise ValueError('"{}" exists in input dataframe'.format(col))

        # Update the existing dataframe
        if inplace:
            for k in features.columns.values:
                df[k] = features[k]
            return df
        else:
